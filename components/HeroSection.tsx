...
const generateBackground = async (data: any) => {
    ...
    if (!response.ok) {
        throw new Error(`Failed to fetch GPT QR code. Status: ${response.status}`);
    }

    const data1 = await response.json();
    return data1.imageUrl;
};

export default function HeroSection() {
    const [inputType, setInputType] = useState<'url' | 'wifi'>('url');
    ...
    const handleGenerateClick = async (e: MouseEvent<HTMLButtonElement>) => {
        ...
        if (inputType === 'wifi' && (!ssid.trim() || !password.trim())) {
            toast.error('Please enter Wi-Fi SSID and password.');
            return;
        }

        setIsLoading(true);
        try {
            const data = inputType === 'url' ? { qr_code_data: url, text_prompt: prompt } : { ssid, password, text_prompt: prompt };
            const qrImageUrl = await generateBackground(data);
    ...
    return (
        <section className="flex justify-center items-center flex-col w-full lg:p-0 p-4">
            <article className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-2 m-4 items-start">
                <div>
                    ...
                    <form>
                        <div className="mb-4 flex items-center space-x-2">
                            <Switch
                                ...
                                onCheckedChange={() => setInputType(inputType === 'url' ? 'wifi' : 'url')}
                            />
                            <span>{inputType === 'url' ? 'URL' : 'Wi-Fi'}</span>
                        </div>
                        ...
                        {inputType === 'url' ? (
                            ...
                            <div className="mb-4 space-y-2">
                                ...
                                />
                                <p>Enter the SSID and password for Wi-Fi connection.</p>
                            </div>
                        )}
